...
def backup(database_name):
    ...
    tar = tarfile.open(BACKUP_PATH + database_name + '_' + BACKUP_TIME + '.tar.gz', "w:gz")
...
def restore(backup_to_restore, download_path, database_name):
    ...
    call(["influxd", "restore", "-database", database_name, "-datadir", INFLUXDB_DATA_DIR, download_path])
